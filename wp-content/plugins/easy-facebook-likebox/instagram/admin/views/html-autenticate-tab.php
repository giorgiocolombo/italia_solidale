...
<div id="mif-general" class="col s12 mif_tab_c slideLeft"> 
   ...
   <a class="mif_auth_btn mif_auth_btn_st btn waves-effect waves-light modal-trigger" href="#mif-authentication-modal">
...
<div class="row auth-row">
    <div class="efbl_all_pages col s12 ">
        ...
        <?php if( $mif_personal_connected_accounts && esf_insta_instagram_type() == 'personal' ){
           ...
           foreach ($mif_personal_connected_accounts as $personal_id => $mif_personal_connected_account) { ?>
            ...
            <ul class="collection with-header"> 
                <li class="collection-header"><h5><?php esc_html_e("Connected Instagram Account", 'easy-facebook-likebox');?></h5> 
                    <a href="#mif-remove-at" class="modal-trigger fta-remove-at-btn tooltipped" data-type="personal" data-position="left" data-delay="50" data-tooltip="<?php esc_html_e("Delete Access Token", 'easy-facebook-likebox');?>"><i class="material-icons">delete_forever</i></a></li>
                    <li class="collection-item li-<?php echo $personal_id ?>">
                       
                      <span class="title"><?php echo $mif_personal_connected_account['username'] ?></span>
                      <p><?php esc_html_e("ID", 'easy-facebook-likebox');?>: <?php echo  $personal_id ?> <i class="material-icons efbl_copy_id tooltipped" data-position="right" data-clipboard-text="<?php echo  $personal_id ?>" data-delay="100" data-tooltip="<?php esc_html_e("Copy", 'easy-facebook-likebox');?>">content_copy</i></p>
                  </li>
              </ul>
       ...
       if( isset($fta_settings['plugins']['facebook']['approved_pages']) && !empty($fta_settings['plugins']['facebook']['approved_pages']) ){ ?>
        ...
        <ul class="collection with-header"> <li class="collection-header"><h5><?php esc_html_e("Connected Instagram Account", 'easy-facebook-likebox');?></h5> <a href="#fta-remove-at" class="modal-trigger fta-remove-at-btn tooltipped" data-position="left" data-delay="50" data-tooltip="<?php esc_html_e("Delete Access Token", 'easy-facebook-likebox');?>"><i class="material-icons">delete_forever</i></a>
        </li>
        ...
        <?php foreach ($fta_settings['plugins']['facebook']['approved_pages'] as $efbl_page){ 
            ...
            if(isset($fta_insta_connected_account->ig_id) && !empty($fta_insta_connected_account->ig_id)){ ?>
                ...
                <li class="collection-item avatar fta_insta_connected_account li-<?php echo $fta_insta_connected_account->ig_id; ?>">
                    ...
                    <a href="https://www.instagram.com/<?php echo $fta_insta_connected_account->username; ?>" target="_blank">
                      <img src="<?php echo $fta_insta_connected_account->profile_picture_url; ?>" alt="" class="circle">
                  </a>          
                  <span class="title"><?php echo $fta_insta_connected_account->name; ?></span>
                  <p><?php echo $fta_insta_connected_account->username; ?> <br> <?php esc_html_e("ID", 'easy-facebook-likebox');?>: <?php echo $fta_insta_connected_account->id; ?> <i class="material-icons efbl_copy_id tooltipped" data-position="right" data-clipboard-text="<?php echo $fta_insta_connected_account->id; ?>" data-delay="100" data-tooltip="<?php esc_html_e("Copy", 'easy-facebook-likebox');?>">content_copy</i></p>
              </li>
          <?php } 
        }
 ...
 <div class="modal-content">
  ...
  <div class="mif-modal-content">
    ...
    <div class="mif-auth-btn-holder">
      ...
      <input class="with-gap" name="mif_login_type" data-url="<?php echo $personal_auth_url; ?>" value="basic" type="radio" id="mif_basic_type" checked />
    ...
    <div class="mif-auth-btn-holder">
      <input class="with-gap" name="mif_login_type" data-url="<?php echo $auth_url ; ?>" value="business" type="radio" id="mif_business_type" />
      <label for="mif_business_type"><?php esc_html_e("Business", 'easy-facebook-likebox');?></label>
      <a href="#" class="mif_info_link"><i class="material-icons">info</i></a>
      <div class="mif_auth_info_holder">
        <p><?php esc_html_e('Used for displaying a user feed from a "Business" or "Creator" Instagram account. A Business or Creator account is required for displaying avatar,bio,comments and likes. See this ', 'easy-facebook-likebox');?><a href="<?php echo esc_url('https://easysocialfeed.com/documentation/how-to-connect-instagram-account-with-facebook-page/'); ?>" target="_blank"> <?php esc_html_e("support guide", 'easy-facebook-likebox');?> </a><?php esc_html_e("to convert personal account to business account.", 'easy-facebook-likebox');?></p>
      </div>
    </div>
    <a href="<?php echo $personal_auth_url; ?>" class="waves-effect waves-light btn mif-auth-modal-btn"><?php esc_html_e("Connect", 'easy-facebook-likebox');?></a>
