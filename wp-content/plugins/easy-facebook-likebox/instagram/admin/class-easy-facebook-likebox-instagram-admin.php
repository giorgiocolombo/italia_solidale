...
if(!class_exists('ESF_Instagram_Admin')){
    ...
    function __construct(){
        ...
        add_action( 'wp_ajax_mif_create_skin', array( $this, 'esf_insta_create_skin' ) );

        add_action( 'wp_ajax_esf_insta_delete_skin', array( $this, 'esf_insta_delete_skin' ) );
    ...
    public function esf_insta_style( $hook ){
        ...
        wp_enqueue_script( 'esf-insta-admin-script', ESF_INSTA_PLUGIN_URL . 'admin/assets/js/esf-insta-admin-script.js', array( 'jquery' ) );
    ...
    * Adds Instagram sub-menu in dashboard
    ...
    function esf_insta_delete_skin(){
        ...
        if ( isset( $skin_deleted ) ) {
            ...
            echo  wp_send_json_success( $returned_arr ) ; wp_die();
        } else {
            echo  wp_send_json_error( __( 'Something Went Wrong! Please try again.', 'easy-facebook-likebox' ) ) ;
            wp_die();
        }
    ...
    function esf_insta_create_skin_url(){
        ...
        if(wp_verify_nonce( $_POST['mif_nonce'], 'mif-ajax-nonce' )){
          ...
          echo  wp_send_json_success(array( __( 'Please wait! We are generating preview for you.', 'easy-facebook-likebox' ), $customizer_url )); wp_die(); 
        
        }else{

            echo  wp_send_json_error( __( 'Something Went Wrong! Please try again.', 'easy-facebook-likebox' ) ) ; wp_die();
        }
    ...
    */
    public function esf_insta_save_access_token(){
        ...
        if(isset($self_decoded_data->error) && !empty($self_decoded_data->error)){
            ...
            echo  wp_send_json_error( $self_decoded_data->error->message );wp_die();
            ...
            if ( isset( $mif_saved ) ) {
                ...
                echo  wp_send_json_success( array( __( 'Successfully Authenticated! Taking you to next step', 'easy-facebook-likebox' ), $mif_accounts_html ) ) ; wp_die();

            } else {
                
                echo  wp_send_json_error( __( 'Something went wrong! Refresh the page and try Again', 'easy-facebook-likebox' ) ) ;wp_die();
