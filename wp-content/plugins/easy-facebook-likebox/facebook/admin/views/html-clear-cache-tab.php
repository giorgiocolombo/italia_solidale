...
<div id="efbl-cached" class="col s12 efbl_tab_c slideLeft">
    ...
    if($efbl_trans_results){
        foreach($efbl_trans_results as $efbl_trans_result){
                        ...
                        */
                       if (strpos($efbl_trans_result->name, 'efbl') !== false && strpos( $efbl_trans_result->name, 'posts' ) !== false && strpos($efbl_trans_result->name, 'timeout') == false) $efbl_trans_posts[$efbl_trans_result->name] =  $efbl_trans_result->value;

                      /*
                        * Checking EFBL exists in transient slug then save that in efbl transient array.
                        */
                      if (strpos($efbl_trans_result->name, 'efbl') !== false && strpos( $efbl_trans_result->name, 'bio' ) !== false && strpos($efbl_trans_result->name, 'timeout') == false) $efbl_trans_bio[$efbl_trans_result->name] =  $efbl_trans_result->value; 
              ...
              if ( $efbl_trans_bio ) { ?>
                <ul class="collection with-header efbl_bio_collection">
                    <li class="collection-header"><h5><?php esc_html_e("Pages Bio", 'easy-facebook-likebox');?></h5></li>
                    ...
                    <?php foreach ( $efbl_trans_bio as $key => $value ) { 
                        ...
                        <li class="collection-item <?php echo $key ?>">
                            ...
                            <a href="javascript:void(0);" data-efbl_collection="efbl_bio_collection" data-efbl_trans="<?php echo $key ?>" class="secondary-content efbl_del_trans"><i class="material-icons">delete</i></a>
                        </div>
                    </li>
        ...
        if($efbl_trans_posts){ ?>
            ...
            <ul class="collection with-header efbl_posts_collection">
                <li class="collection-header"><h5><?php esc_html_e("Feeds", 'easy-facebook-likebox');?></h5></li>
                ...
                <?php foreach($efbl_trans_posts as $key => $value){  
                    ...
                    <li class="collection-item <?php echo $key ?>">
                        <div><?php echo $page_name ?> <?php if( $filter ): ?>(<?php echo ucfirst( $filter ); ?>) <?php endif; ?> 
                            <a href="javascript:void(0);" data-efbl_trans="<?php echo $key ?>" class="secondary-content efbl_del_trans"><i class="material-icons">delete</i></a>
                        </div>
                    </li>
