...
class IO_Theme_Templates {
    ...
    public static function set_all_templates() {
        $all_templates = WC_Admin_Status::scan_template_files( WC()->plugin_path() . '/templates' );
	if ( function_exists(  'WC_PB' ) ) {
		$all_templates = array_merge( WC_Admin_Status::scan_template_files( WC_PB()->plugin_path() . '/templates' ), $all_templates );
	}
        self::$all_templates = apply_filters( 'woo-edit-template-all_templates', $all_templates );
    }
	...
	add_action( 'wp_ajax_wooet_dismiss_rate', array( $this, 'dismiss_rate' ) );
	add_action( 'wp_ajax_wooet_temporary_dismiss_rate', array( $this, 'temporary_dismiss_rate' ) );
	...
	/**
	 * Temporary hide admin notice
	 */
	public function temporary_dismiss_rate() {
		update_user_meta( get_current_user_id(), 'wooet_dismiss_time_rate_plugin', time() + WEEK_IN_SECONDS );
		exit;
	}
	/**
	 * Completely hide admin notice
	 */
	public function dismiss_rate() {
		update_user_meta( get_current_user_id(), 'wooet_dismiss_rate_plugin', '1' );
		exit;
	}
	...
	/**
	 * Show admin notice for rate plugin
	 */
	public function rate_plugin() {
		$dismiss = get_user_meta( get_current_user_id(), 'wooet_dismiss_rate_plugin', true );
		$dismiss_time = get_user_meta( get_current_user_id(), 'wooet_dismiss_time_rate_plugin', true );

		if ( !$dismiss && ( !$dismiss_time || $dismiss_time < time() ) ) {
			wp_enqueue_script( 'wooet_template_edit', plugins_url( 'asset/js/template_edit.js', __FILE__ ), array( 'jquery' ), false, true );
			?>
			<div class="notice notice-success is-dismissible wooet-rate_plugin-notice" >
				<p><?php printf(
					__( 'If you like %1$s please leave us a %2$s rating. A huge thanks in advance!', 'woocommerce' ),
					sprintf( '<strong>%s</strong>', esc_html__( 'Edit WooCommerce Templates' ) ),
					'<a href="https://wordpress.org/support/plugin/woo-edit-templates/reviews?filter=5#new-post" target="_blank" class="wc-rating-link wooet-rating" data-rated="' . esc_attr__( 'Thanks :)', 'woocommerce' ) . '">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'
				); ?></p>
			</div>
			<?php
		}
